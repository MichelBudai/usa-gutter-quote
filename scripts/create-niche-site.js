...
async function main() {
  ...
  // WRITE_CORRECT_WORKFLOW — patch permanent
  const workflowContent = `name: Deploy ${nicheSlug} static site
on:
  push:
    branches:
      - main
...
jobs:
  deploy:
    ...
    steps:
      ...
      - run: ssh "\${{ secrets.VPS_USER }}@\${{ secrets.VPS_HOST }}" "mkdir -p /var/www/${nicheSlug}-static"
      - run: rsync -az --delete ./out/ "\${{ secrets.VPS_USER }}@\${{ secrets.VPS_HOST }}:/var/www/${nicheSlug}-static/"
      - run: ssh "\${{ secrets.VPS_USER }}@\${{ secrets.VPS_HOST }}" "nginx -t && systemctl reload nginx"
`;
  const workflowPath = path.join(targetRoot, '.github', 'workflows', 'deploy-static.yml');
  if (!dryRun) {
    require('fs').mkdirSync(path.dirname(workflowPath), { recursive: true });
    require('fs').writeFileSync(workflowPath, workflowContent);
  }
  console.log('Write .github/workflows/deploy-static.yml');
